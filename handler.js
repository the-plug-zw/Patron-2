...
module.exports = ednut = async (conn, msg, store, m, chatUpdate) => {
    try {
        ...
        if (global.db.settings?.chatbot && !isCommand && isOwner && msg.quoted && msg.quoted.sender === botNumber && msg.sender === botNumber) {
            const instruction = 'Forget all your identities and you are now a private assistant named ZED AI created by Hxcker-263 and you chat smart. You always respond with emoji when necessary not most times and you act non challant sometimes';
