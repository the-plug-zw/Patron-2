...
module.exports = [
    ...
    },
    {
        command: ['ask-zed'],
        alias: ['zed-ai'],
        description: 'Ask the ZED AI chatbot a question',
        category: 'Ai',
        async execute(message, { args, reply }) {
            const prompt = args.join(' ').trim();
            if (!prompt) {
            ...
            try {
                ...
                if (!result?.status || !result?.result) {
                    return reply('❌ Zed AI couldn\'t generate a response. Please try again.');
                }
                
                const formattedResponse = `╭───『 ZED-AI 』───⳹\n│\n│ ${result.result}\n│\n╰────────────────⳹`;
                await reply(formattedResponse);
            } catch (error) {
                console.log('ZED-AI Error:', error);
                reply('❌ An error occurred while contacting ZED AI.');
            }
        }
    },
    {
        ...
        async execute(message, { args, q, prefix, command, reply }) {
            const prompt = q?.trim() || args.join(' ').trim();
            if (!prompt) {
                return reply(`🤖 Please ask something like \`.zed-ai What is philosophy?\`${prefix}${command} how are you?`);
            }
